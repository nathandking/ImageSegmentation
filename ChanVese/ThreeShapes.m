...
% Three noisy/blurred shapes image
clear all
close all
tic

%% Read and plot initial image
u0=imread('../Images/BlurredThreeShapes.jpg');
[M,N]=size(u0);
u0 = double(u0);
...
y0=N/2;
r0=10;
Phi0 = -sqrt((Qx-x0).^2+(Qy-y0).^2)+r0;

%% Parameters

lambda=0.0001;   % tunable parameter
h=1.;   %  space step size
IterMax=2;   %  number of iterations
